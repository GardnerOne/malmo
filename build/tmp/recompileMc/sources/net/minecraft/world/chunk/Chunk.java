...
public class Chunk
{
    ...
    private int getBlockLightOpacity(int x, int y, int z)
    {
        ...
        return !loaded ? state.getLightOpacity() : state.getLightOpacity(world, new BlockPos(x << 4 | x & 15, y, z << 4 | z & 15));
    }
    ...
    public int getLightFor(EnumSkyBlock type, BlockPos pos)
    {
        ...
        int k = pos.getZ() & 15;
        ExtendedBlockStorage extendedblockstorage = this.storageArrays[j >> 4];
        return extendedblockstorage == NULL_BLOCK_STORAGE ? (this.canSeeSky(pos) ? type.defaultLightValue : 0) : (type == EnumSkyBlock.SKY ? (!this.world.provider.hasSkyLight() ? 0 : extendedblockstorage.getSkyLight(i, j & 15, k)) : (type == EnumSkyBlock.BLOCK ? extendedblockstorage.getBlockLight(i, j & 15, k) : type.defaultLightValue));
    }
    ...
    public void addEntity(Entity entityIn)
    {
        ...
        {
            LOGGER.warn("Wrong location! ({}, {}) should be ({}, {}), {}", new Object[] {Integer.valueOf(i), Integer.valueOf(j), Integer.valueOf(this.x), Integer.valueOf(this.z), entityIn});
            entityIn.setDead();
        }
